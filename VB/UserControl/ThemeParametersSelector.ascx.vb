...
Partial Public Class UserControl_ThemeParametersSelector
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
		If Utils.CanChangeBaseColor Then
			BaseColorEditor.DataSource = Utils.CustomBaseColors.Select(Function(c) New With {
				Key .Value = c,
				Key .ImageUrl = Utils.GetColoredSquareUrl(c)
			})
			...
			End If
		End If
		FontEditor.DataSource = Utils.GetFontFamiliesDataSource().Select(Function(i) New With {
			Key .Value = i.Value,
			Key .Text = i.Text,
			Key .ImageUrl = ""
		})
